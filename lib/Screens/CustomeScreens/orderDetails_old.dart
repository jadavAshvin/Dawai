...
class _OrderDetailsState extends State<OrderDetails> {
  final OrderDetailsController orderDerailsController = Get.put(OrderDetailsController());
  int ticks;
              ...
              children: [
                ...
                Padding(
                  ...
                  child: widget.screenlag == 0
                      ? Obx(() {
                          addressVal = orderDerailsController.address.value.trim().isEmpty ? "" : "${orderDerailsController.address.value},";
                          building = orderDerailsController.house_building.value.trim().isEmpty ? "" : "${orderDerailsController.house_building.value},";
                          apartment = orderDerailsController.appartment_number.value.trim().isEmpty ? "" : "${orderDerailsController.appartment_number.value},";
                          way = orderDerailsController.way_number.value.trim().isEmpty ? "" : "${orderDerailsController.way_number.value},";
                          street = orderDerailsController.street_name.value.trim().isEmpty ? "" : "${orderDerailsController.street_name.value},";
                          city = orderDerailsController.city.value.trim().isEmpty ? "" : "${orderDerailsController.city.value},";
                          gov = orderDerailsController.governorate.value.trim().isEmpty ? "" : "${orderDerailsController.governorate.value}";
                          if (orderDerailsController.created_date.isNotEmpty) {
                            selectedDate = orderDerailsController.created_date.split("-")[2] + "/" + orderDerailsController.created_date.split("-")[1] + "/" + orderDerailsController.created_date.split("-")[0];
                          }
                          String orderDate = "";
                          if (orderDerailsController.order_status_id.value == "4") {
                            orderDate = getLabel("on") + " " + orderDerailsController.dilvery_Date.value + " " + orderDerailsController.dilvery_time.value;
                          }
                          ...
                          return orderDerailsController.invoiceno.isEmpty
                              ...
                              : Column(
                                  ...
                                  children: [
                                    setView(getLabel("order_no") + " : ${orderDerailsController.invoiceno}"),
                                    setView(getLabel("order_date") + " : $selectedDate"),
                                    setView(getLabel("mrn_no") + " : ${orderDerailsController.mrn_no}"),
                                    setView(getLabel("mobile_no") + " : ${orderDerailsController.mobile_no}"),
                                    setView(getLabel("nearest_med_center") + " : ${orderDerailsController.nearest_med_center}"),
                                    ...
                                    customerContent(orderDerailsController.order_status_name.toString() + " " + orderDate, orderDerailsController.color_code.toString()),

                                    orderDerailsController.not_del_content != "" ? setView(getLabel("status_message") + " : ${orderDerailsController.not_del_content}") : Container(),
                                    widget.screenName == "MediacalCenter" ? Container() : setView(getLabel("order_pin") + " : ${orderDerailsController.order_pin}"),
                                    Container(),
                          ...
                          return orderDerailsController.invoiceno.isEmpty
                              ...
                              : Column(
                                  ...
                                  children: [
                                    ...
                                    setView(getLabel("delivery_address") + ": " + addressVal + way + building + apartment + street + city + gov),
                                    customerContent(orderDerailsController.order_status_name.toString() + " " + orderDate, orderDerailsController.color_code.toString()),
                                    orderDerailsController.order_status_name == "In Transit" ? driverContent(context) : Container(),
                                    orderDerailsController.not_del_content != "" ? setView(getLabel("status_message") + " : ${orderDerailsController.not_del_content}") : Container(),
                                    Center(
                                      child: Text(
                                        getLabel("order_status_heading"),
    ...
    return Column(
      ...
      children: [
        Text(
          ...
          style: TextStyle(fontSize: 18, color: colorConvert(colorCode), fontWeight: FontWeight.bold),
        ),
        Divider(
          thickness: 0.5,
          color: black,
    ...
    return Column(
      children: [
        ...
        CustomButton(getLabel("get_direction"), () async {
          String origin = "${getPrefValue(Keys.USER_LOCATION)}"; // lat,long like 123.34,68.56
          String destination = "${orderDerailsController.lat_long.value}";
          if (Platform.isAndroid) {
            final AndroidIntent intent = new AndroidIntent(action: 'action_view', data: Uri.encodeFull("https://www.google.com/maps/dir/?api=1&origin=" + origin + "&destination=" + destination + "&travelmode=driving&dir_action=navigate"), package: 'com.google.android.apps.maps');
            intent.launch();
          } else {
  ...
  Widget statusView(BuildContext context) {
    return Column(
      ...
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        orderDerailsController.order_status_name.value == "Not Delivered" || orderDerailsController.order_status_name.value == "Order Delivered"
            ? Container()
            /*: widget.notShowlag == 1
            ? Container()*/
            : Row(
                      ...
                      child: RaisedButton(
                              ...
                              builder: (context) {
                                return AlertDialog(
                                  ...
                                  content: TextField(
                                    ...
                                    decoration: InputDecoration(hintText: getLabel("enter_order_pin")),
                                  ),
                                  actions: <Widget>[
                                    FlatButton(
                                      color: buttonColor,
                                      ...
                                      onPressed: () {
                                        if (orderDerailsController.order_pin.value == orderDerailsController.orderPinController.text) {
                                          Get.back();
                                          orderDerailsController.updateOrderStatus(orderDerailsController.invoiceno.value, "4", "");
                                          orderDerailsController.orderPinController.clear();
                                        } else {
                                          mySnackbar(title: '${getLabel("error")}', description: getLabel("enter_valid_pin"));
                        ...
                        child: Padding(
                            ...
                            child: Text(
                              ...
                              style: textFieldTextBoldStyle(16, white),
                            )),
                      ),
                    ),
                  ),
                          ...
                          Get.defaultDialog(
                            ...
                            content: Column(
                              children: [
                                InkWell(
                                    onTap: () {
                                      orderDerailsController.updateOrderStatus(orderDerailsController.invoiceno.value, "5", getLabel("not_del_1"));
                                      Get.back();
                                    },
                                    child: Card(
                                      ...
                                      child: Padding(
                                        ...
                                        child: Text(getLabel("not_del_1"), style: textFieldTextStyle(16, black)),
                                      ),
                                    )),
                                SizedBox(
                                  height: 10,
                                ...
                                InkWell(
                                    onTap: () {
                                      orderDerailsController.updateOrderStatus(orderDerailsController.invoiceno.value, "5", getLabel("not_del_2"));
                                      Get.back();
                                    },
                                    child: Card(
                                      elevation: 5,
                                      child: Padding(
                                        padding: const EdgeInsets.all(8.0),
                                        child: Text(getLabel("not_del_2"), style: textFieldTextStyle(16, black)),
                                      ),
                                    )),
                                SizedBox(
                                  height: 10,
                                ...
                                InkWell(
                                    onTap: () {
                                      orderDerailsController.updateOrderStatus(orderDerailsController.invoiceno.value, "5", getLabel("not_del_3"));
                                      Get.back();
                                    },
                                    child: Card(
                                      elevation: 5,
                                      child: Padding(
                                        padding: const EdgeInsets.all(8.0),
                                        child: Text(getLabel("not_del_3"), style: textFieldTextStyle(16, black)),
                                      ),
                                    )),
                                SizedBox(
                                  height: 10,
                                ...
                                InkWell(
                                    onTap: () {
                                      orderDerailsController.updateOrderStatus(orderDerailsController.invoiceno.value, "5", getLabel("not_del_4"));
                                      Get.back();
                                    },
                                    child: Card(
                                      elevation: 5,
                                      child: Padding(
                                        padding: const EdgeInsets.all(8.0),
                                        child: Text(getLabel("not_del_4"), style: textFieldTextStyle(16, black)),
                                      ),
                                    )),
                                SizedBox(
                                  height: 10,
                                ...
                                InkWell(
                                    onTap: () {
                                      orderDerailsController.updateOrderStatus(orderDerailsController.invoiceno.value, "5", getLabel("not_del_5"));
                                      Get.back();
                                    },
                                    child: Card(
                                      elevation: 5,
                                      child: Padding(
                                        padding: const EdgeInsets.all(8.0),
                                        child: Text(getLabel("not_del_5"), style: textFieldTextStyle(16, black)),
                                      ),
                                    )),
                                SizedBox(
                                  height: 10,
